...
def upgrade():
    ...
    conn.execute(
        "DELETE FROM public.tenantconfig a USING public.tenantconfig b WHERE a.id < b.id AND a.tenant_id = b.tenant_id"
    )

    # next, add the uniqueness constraint to the tenant_id field of the TenantConfig table.
    conn.execute(
        "alter table public.tenantconfig add constraint uq_tenantconfig_tenant_id unique (tenant_id)"
    )
