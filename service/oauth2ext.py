...
class OAuth2ProviderExtension(object):
    ...
    def __init__(self, tenant_id, is_local_development=False, idp_id_for_multi=None):
        ...
        elif self.ext_type == 'vdj':
            self.client_id = self.custom_idp_config_dict.get('vdj').get('client_id')
            self.client_key = self.custom_idp_config_dict.get('vdj').get('client_secret')
            # initial redirect URL; used to start the oauth flow and log in the user
            self.identity_redirect_url = self.custom_idp_config_dict.get('vdj').get('identity_redirect_url')
            # URL to use to exchange the code for an qccess token
            self.oauth2_token_url = self.custom_idp_config_dict.get('vdj').get('oauth2_token_url')
            # URL to look up user info from token
            self.user_info_url = self.custom_idp_config_dict.get('vdj').get('user_info_url')
            logger.debug("properties set of vdj")
