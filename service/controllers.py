...
import requests
import sqlalchemy
from flask import (Response, g, jsonify, make_response, redirect,
...
from service.ldap import (check_username_password, get_tenant_user,
                          list_tenant_users)
from service.mfa import (call_mfa, check_mfa_expired, check_sms, needs_mfa,
                         send_sms)
from service.models import (AccessTokens, AuthorizationCode, Client,
                            DeviceCode, RefreshTokens, TenantConfig, Token,
                            TokenRequestBody, db, tenant_configs_cache,
...
class ClientsResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logger.debug(
                f"got exception trying to commit client object to db. Exception: {e}"
            )
...
class ClientResource(Resource):
    ...
    def put(self, client_id):
        ...
        logger.debug(
            f"got past checks for unsupported fields. using tenant_id: {g.request_tenant_id}; client_id: {client_id}"
        )
        g.tenant_id = g.request_tenant_id
        g.username = g.request_username
        clients = Client.query.filter_by(
    ...
    def get(self):
        ...
        if tenant_configs_cache.get_custom_oa2_extension_type(tenant_id=tenant_id):
            raise errors.ResourceError(
                f"This endpoint is not available in the {tenant_id} tenant. The profiles "
        ...
        resp = utils.ok(
            result=[u.serialize for u in users], msg="Profiles retrieved successfully."
        )
...
def _handle_userinfo_request(request, oidc=False):
    ...
    if custom_oa2_extension_type and not custom_oa2_extension_type == "ldap":
        logger.debug(
            f"Using custom auth for userinfo; custom_oa2_extension_type: {custom_oa2_extension_type}"
        )
...
class TenantConfigResource(Resource):
    ...
    def put(self):
        ...
        if not config:
            raise errors.ResourceError(
                f"Config for tenant {tenant_id} does not exist. Contact system administrators."
            )
        logger.debug(
        ...
        if new_allowable_grant_types:
            try:
                ...
                raise errors.ResourceError(
                    f"Invalid allowable_grant_type ({new_allowable_grant_types}) -- must be "
                    f"JSON serializable"
                )
        ...
        if new_custom_idp_configuration:
            try:
                ...
                logger.debug(
                    f"got exception trying to parse new_custom_idp_configuration; e: {e}"
                )
                raise errors.ResourceError(
                    f"Invalid new_custom_idp_configuration ({new_custom_idp_configuration}) -- "
        ...
        try:
            ...
            logger.debug(
                f"got exception trying to commit updated tenant config object to db. Exception: {e}"
            )
            msg = utils.get_message_from_sql_exc(e)
            logger.debug(f"returning msg: {msg}")
...
class OIDCjwksResource(Resource):
    ...
    def get(self):
        ...
        return jsonify(
            json_response
        )  # utils.ok(result=metadata, msg='OAuth OIDC metadata retrieved successfully.')
...
def check_client(use_session=False):
    ...
    if not tenant_id in conf.tenants:
        logout()
        raise errors.ResourceError(
            f"This application is not configured to serve the requested tenant {tenant_id}."
        )
    if use_session:
        ...
        if not client_id:
            logger.debug(
                "The use_session was true, but we didn't find a client in the session so we are looking in query parameters."
            )
    ...
    if not client.callback_url == client_redirect_uri:
        ...
        logger.debug(
            f"redirect_uri query parameter does not match registered callback_url for the client. redirect_uri: {client_redirect_uri}; callback_url: {client.callback_url}"
        )
        raise errors.ResourceError(
...
class AuthorizeResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logger.debug(
                f"did not find username in session; this is an error. raising error. session: {session};"
            )
            raise errors.ResourceError(
        ...
        if not approve:
            ...
            headers = {"Content-Type": "text/html"}
            context = {
                "error": f"To proceed with authorization application {client_display_name}, you "
...
class OAuth2ProviderExtCallback(Resource):
    ...
    def get(self):
        ...
        if oa2ext.ext_type == "multi_idps":
            ...
            if not idp_id:
                raise errors.ResourceError(
                    f"Unable to process callback from Identity provider. Details: idp_id missing from session."
                )
...
def _handle_tokens_request(request, oidc=False):
    ...
    if oidc or not oidc:
        ...
        if not grant_type:
            raise errors.ResourceError(
                msg=f"Missing the required grant_type parameter."
            )
        ...
        if "localhost" in request.base_url:
            logger.debug(
                "localhost was in the request.base_url so we are looking for X-Tapis-Local-Tenant header.."
            )
        ...
        if grant_type == "device_code":
            ...
            if client_id != db_code.client_id:
                ...
                raise errors.ResourceError(
                    msg="Client IDs do not match between device code and passed in client_id"
                )
        ...
        if not client_id and not client_key and grant_type == "password":
            logger.debug(
                "Allowing the password grant request even though the auth header missing."
            )
        # check that client is in db
            ...
            if not client:
                # todo -- remove session
                logger.debug(
                    f"Client with id {client_id} and key {client_key} not found on tenant {tenant_id}."
                )
                raise errors.ResourceError(
        ...
        if grant_type == "password":
            ...
            if not username or not password:
                raise errors.ResourceError(
                    "Missing required payload data; username and password are required for "
            ...
            if not token_type == "refresh":
                ...
                raise errors.ResourceError(
                    f"Invalid token type. The refresh_token grant type required a token of type "
            ...
            if not access_token_claims:
                logger.error(
                    f"Got a refresh token that did NOT have an access_token claim. claims: {claims}"
                )
                raise errors.ResourceError(
                    "Invalid refresh_token format; this token was missing the access_token "
                    "claim."
                )
            ...
            if not client_id == client_id_claim:
                msg = (
                    f"client_id from header ({client_id}) does not match the client_id in the token claim "
                    f"({client_id_claim})."
                )
                logger.debug(msg)
                raise errors.ResourceError(msg)
        ...
        try:
            ...
            logger.error(
                f"Got an unexpected AttributeError trying to parse tokens response; e: {e}"
            )
            raise errors.ResourceError(
        ...
        try:
            ...
            logger.debug(
                f"got exception trying to commit access_token object to db. Exception: {e}"
            )
            msg = utils.get_message_from_sql_exc(e)
            logger.debug(f"returning msg: {msg}")
            raise errors.ResourceError(f"Invalid POST data; {msg}")
...
class RevokeTokensResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logger.error(
                f"Got exception trying to call the tokens api to revoke a token; details: {e}"
            )
            raise errors.ResourceError(
                msg=f"Unexpected error trying to revoke the token: debug data: {e}."
            )
        logger.info(
            f"Token has been revoked with the Tokens API, will now update our table."
        )
        ...
        try:
            ...
            logger.error(
                f"could not get claims from revoked token and therefore could not update the "
        ...
        if revoked_token_claims:
            ...
            try:
                ...
                logger.error(
                    f"could not get token_typ and jti claims from revoked token and therefore "
                ...
                if token_type == "access":
                    ...
                    if not access_token:
                        ...
                        try:
                            db.session.commit()
                            logger.info(
                                f"access token with jit {jti} revoked, and revoked status added to table."
                            )
                        except Exception as e:
                            logger.error(
                                f"could not commit update to revoked access token; e: {e}"
                            )
                else:
                    ...
                    if not refresh_token:
                        ...
                        try:
                            db.session.commit()
                            logger.info(
                                f"refresh token with jit {jti} revoked, and revoked status added to table."
                            )
                        except Exception as e:
                            logger.error(
                                f"could not commit update to revoked refresh token; e: {e}"
                            )
        else:
            logger.error(
                f"insecure_decode_jwt_to_claims did not return claims for the token: {token_str}"
            )
...
def get_tokenapp_client(tenant_id=None):
    ...
    if not tenant_id:
        ...
        raise errors.ResourceError(
            msg=f"The tenant could not be established from the session."
        )
        ...
        if token:
            ...
            if has_valid_session:
                try:
                    ...
                    logger.error(
                        f"Got exception trying to get username out of userinfo endpoint response object. This should never happen. Setting username to 'Not available'. Details: {e}; user_info: {user_info}"
                    )
...
class WebappTokenGen(Resource):
    ...
    def get(self):
        ...
        if not username:
            logger.error(
                "GET request to /v3/oauth2/webapp/callback made but WebappTokenGen could not "
                "find username in the session! "
            )
            raise errors.ResourceError(
                msg=f"The username could not be established from the session."
            )
        tenant_id = g.request_tenant_id
        ...
        try:
            ...
            logger.error(
                f"Got exception trying to POST to /v3/oauth2/tokens endpoint. Exception: {e}"
            )
            raise errors.ResourceError(
                "Failure to generate an access token; please try again later."
            )
        ...
        try:
            ...
            logger.error(
                f"Got exception trying to parse JSON from POST to /v3/tokens endpoint. Exception: {e}"
            )
            raise errors.ResourceError(
                "Failure to generate an access token; please try again later."
            )
        ...
        logger.debug(
            f"Made request successfully and got JSON. Now parsing JSON data: {json_resp}"
        )
...
class WebappLogout(Resource):
    ...
    def get(self):
        ...
        logout_from_webapp()
        logger.debug(
            f"User has been logged out of webapp; remaining session keys: {session.keys()}"
        )
