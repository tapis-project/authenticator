...
class LogoutResource(Resource):
    ...
    def get(self):
        ...
        # if another application is redicrecting here, then automatically log out the user
        # without showing them the form and simply issue the redirect
        redirect_url = request.args.get("redirect_url")
        if redirect_url:
            if not redirect_url.startswith("https"):
                logger.info(
                    f"Invalid redirect_url {redirect_url}; must start with HTTPS."
                )
            else:
                logger.debug(
                    f"Got redirect_url {redirect_url}; issuing logout directly"
                )
                logout()
