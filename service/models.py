...
try:
    full_db_url = (
        f"postgresql://{conf.postgres_user}:{conf.postgres_password}@{conf.sql_db_url}"
    )
...
class AuthorizationCode(db.Model):
    ...
    client_id = db.Column(
        db.String(80), db.ForeignKey("clients.client_id"), unique=False, nullable=False
    )
    client_key = db.Column(db.String(80), unique=False, nullable=False)
    ...
    def compute_expiry(cls):
        ...
        return datetime.datetime.utcnow() + datetime.timedelta(
            seconds=AuthorizationCode.CODE_TTL
        )
    ...
    def validate_code(cls, tenant_id, code, client_id, client_key):
        ...
        code_result = cls.query.filter_by(
            tenant_id=tenant_id, code=code, client_id=client_id, client_key=client_key
        ).first()
        if not code_result:
        ...
        if (
            not datetime.datetime.utcnow()
            <= code_result.expiry_time + datetime.timedelta(seconds=6)
        ):
...
class DeviceCode(db.Model):
    ...
    def generate_verification_uri(cls, tenant, client_id, BASE_URL=BASE_URL):
        return BASE_URL.format(tenant) + "/v3/oauth2/device?client_id={}".format(
            client_id
        )
    ...
    def validate_code(cls, code):
        ...
        if (
            ...
            raise InvalidDeviceCodeError(
                msg="device code has expired and is now deleted."
            )
        return code_result
...
class LdapUser(object):
    ...
    def save(self, conn):
        ...
        if (
            not result
            and hasattr(conn.result, "get")
            and "entryAlreadyExists" in conn.result.get("description")
        ):
...
class Token(object):
    ...
    def __init__(
        self, client_id=None, client_secret=None, username=None, password=None
    ):
        """
        oauth2 token
        """
    ...
    def get_derived_values(cls, data):
        ...
        result["client_key"] = getattr(data, "client_key", None) or getattr(
