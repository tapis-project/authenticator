...
def teardown_module():
    ...
    with app.app_context():
        models.AuthorizationCode.query.filter_by(
            tenant_id=TEST_TENANT_ID,
            client_id=TEST_CLIENT_ID,
            client_key=TEST_CLIENT_KEY,
        ).delete()
...
def test_get_metadata(client):
    result = client.get(
        "http://localhost:5000/v3/oauth2/.well-known/oauth-authorization-server"
    )
    assert result.status_code == 200
...
def test_get_admin_config(client, tapis_service_jwt, init_db):
    with client:
        ...
        response = client.get(
            "http://localhost:5000/v3/oauth2/admin/config", headers=header
        )
        ...
        tenant_config = [
            conf for conf in tenant_configs if conf.tenant_id == TEST_TENANT_ID
        ][0]
...
def test_update_admin_config(client, tapis_service_jwt):
    with client:
        ...
        current_config = [
            d
            for d in tenant_configs_cache.load_tenant_config_cache()
            if d.tenant_id == TEST_TENANT_ID
        ][0].serialize

        # just change one thing
        payload = {"impers_oauth_client_id": "TEST"}
        # make request
        header = {
            "X-Tapis-Token": tapis_service_jwt,
            "X-Tapis-Tenant": TEST_TENANT_ID,
...
def insert_test_client(client):
    ...
    new_client = models.Client.query.filter_by(
        tenant_id=TEST_TENANT_ID, client_id=new_client_id, client_key=TEST_CLIENT_KEY
    ).first()
...
def test_authenticator_create_clients(
    ...
    with client:
        ...
        check_clients_table(
            new_client_id,
            TEST_CLIENT_REDIRECT_URI,
            "A Test Client",
            "This is a client just for testing",
        )
        # cleanup
        ...
        )  # fail if we can't find the client. This means the test didn't work
...
def test_password_grant_invalid_client(client, init_db):
    with client:
        ...
        auth_header = {
            "Authorization": get_basic_auth_header("bad_client_id", "bad_client_key")
        }
...
def test_password_grant_missing_username(client, init_db):
    with client:
        ...
        payload = {"grant_type": "password", "password": "abcd"}
        response = client.post(
            "http://localhost:5000/v3/oauth2/tokens",
            headers=auth_header,
            data=json.dumps(payload),
...
def test_password_grant_missing_password(client, init_db):
    with client:
        ...
        payload = {"grant_type": "password", "username": "abcd"}
        response = client.post(
            "http://localhost:5000/v3/oauth2/tokens",
            headers=auth_header,
            data=json.dumps(payload),
...
def test_password_grant_invalid_uppercase_user(client, init_db):
    # ldap servers are case insensitive, but the ldap.py bind code checks for upper case letters in the
...
def test_password_grant_invalid_spaces_user(client, init_db):
    # ldap servers ignore spaces in a username, but the ldap.py bind code checks for
    # spaces in the username and rejects it if any appear.
    with client:
        auth_header = {
            "Authorization": get_basic_auth_header(TEST_CLIENT_ID, TEST_CLIENT_KEY)
        }
...
def test_revoke_token(client, init_db):
    ...
    with client:
        ...
        check_refresh_token_table(
            refresh_token_claims, "password", True, TEST_CLIENT_ID
        )
...
def test_get_userinfo(client, tapis_jwt):
    with client:
        header = {"X-Tapis-Token": tapis_jwt}
        result = client.get("http://localhost:5000/v3/oauth2/userinfo", headers=header)
        assert result.status_code == 200
...
def test_list_profiles(client, tapis_jwt):
    with client:
        header = {"X-Tapis-Token": tapis_jwt}
        result = client.get("http://localhost:5000/v3/oauth2/profiles", headers=header)
        assert result.status_code == 200
...
def test_get_profile(client, tapis_jwt):
    with client:
        header = {"X-Tapis-Token": tapis_jwt}
        result = client.get(
            f"http://localhost:5000/v3/oauth2/profiles/{TEST_USERNAME}", headers=header
        )
        assert result.status_code == 200
...
def test_authorization_code(client, init_db):
    ...
    with client:
        ...
        auth_code = models.AuthorizationCode.query.filter_by(
            tenant_id=TEST_TENANT_ID,
            client_id=TEST_CLIENT_ID,
            client_key=TEST_CLIENT_KEY,
            username=TEST_USERNAME,
...
def test_authorization_code_grant(client, init_db):
    with client:
        ...
        assert (
            claims["tapis/access_token"]["tapis/redirect_uri"]
            == TEST_CLIENT_REDIRECT_URI
        )
...
def test_get_device_code(client):
    with client:
        ...
        response = client.post(
            "http://localhost:5000/v3/oauth2/device/code",
            data=json.dumps(data),
            content_type="application/json",
        )
        # print(response.json)
        ...
        check_device_code_table(
            TEST_CLIENT_ID, user_code, device_code, verification_url, "Created"
        )
...
def test_exchange_device_code(client):
    ...
    verification_url = (
        models.DeviceCode.generate_verification_uri(
            TEST_TENANT_ID, TEST_CLIENT_ID, BASE_URL="https://localhost:5000"
        ),
    )
    try:
    ...
    header = {"X-Tapis-Local-Tenant": "dev", "content-type": "application/json"}
    response = client.post(
        "http://localhost:5000/v3/oauth2/tokens", data=json.dumps(body), headers=header
    )
